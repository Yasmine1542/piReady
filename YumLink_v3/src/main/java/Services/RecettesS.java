...
public class RecettesS implements Service<Recettes>{
    ...
    public void ajouter(Recettes recette) throws SQLException{
        String sql = "INSERT INTO recettes (nom, description,imgSrc,videoSrc, calorie , protein) VALUES (?, ? , ?, ?, ?, ?)";
        try (PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
            ...
            preparedStatement.setString(4, recette.getVideoSrc());
            preparedStatement.setInt(5, recette.getCalorie());
            preparedStatement.setInt(6, recette.getProtein());
    ...
    public void modifier(Recettes recettes) throws SQLException {
        String sql = "UPDATE recettes SET nom = ?, description = ?, calorie = ?, protein = ?, imgSrc = ?, videoSrc = ? WHERE id_r = ?";
        ...
        pstmt.setString(6, recettes.getVideoSrc());
        pstmt.setInt(7, recettes.getId_r());
        pstmt.executeUpdate();
        pstmt.close();
